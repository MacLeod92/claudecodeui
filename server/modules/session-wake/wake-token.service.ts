 ...
 * need to). Minted server-side when a background command is spawned (the
 * `canUseTool`/`PreToolUse` rewrite in
 * modules/providers/list/claude/claude-runtime.provider.js), scoped to the one
 * session that spawned it, and consumed on first use.
 */
