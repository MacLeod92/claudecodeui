...
import { wakeSession, type SessionWakeRuntimeGateway } from './session-wake.service.js';
 ...
 * a live socket.
 *
 * `runtime` comes from the same composition root (server/index.ts) that wires
 * the identical dispatcher into the chat websocket server.
 */
export function createSessionWakeRoutes(
  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      const result = await wakeSession(runtime, { sessionId, prompt, userId });
