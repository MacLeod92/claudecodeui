...
test('getCurrentActiveModel maps raw JSONL model ids to catalog aliases', async () => {
  await withIsolatedDatabase(async () => {
    ...
    try {
      ...
      // JSONL transcripts record the raw Anthropic API model id, not the short
      // CLI alias, and this one also carries the 1M-context suffix.
      await writeFile(
        jsonlPath,
        `${JSON.stringify({
          type: 'assistant',
          sessionId: appSessionId,
          message: { model: 'claude-opus-4-6-1m' },
        })}\n`,
        'utf8',
      );
      ...
      assert.equal(activeModel.model, 'opus[1m]');
    } finally {
      await rm(tempDirectory, { recursive: true, force: true });
    }
  });
